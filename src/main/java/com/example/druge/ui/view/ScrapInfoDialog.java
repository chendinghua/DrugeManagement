...
public class ScrapInfoDialog extends Dialog{

    public ScrapInfoDialog(@NonNull Context context) {
        super(context);
    } public ScrapInfoDialog(Context context, int theme) {
        super(context, theme);
    }
    ...
    public static class Builder {
        ...
        private View layout;
        private ScrapInfoDialog dialog;


      /*  private TextView productId;       //产品id
        private TextView productCode;     //产品编号
        private TextView productType;     //产品类型
        private TextView productLot;      //批次号
        private TextView productPutOnTime;//入库时间
        private TextView productStatus;   //产品状态
        private TextView productStock;    //产品库位信息
        private TextView productNum;      //产品库存数量*/

        private Stockdrug stockDrug;

        public Builder(Context context) {
            //这里传入自定义的style，直接影响此Dialog的显示效果。style具体实现见style.xml
            dialog = new ScrapInfoDialog(context, R.style.Dialog);
         ...
         * @return
         */
        public ScrapInfoDialog createSingleButtonDialog() {
            showSingleButton();
        ...
        public Builder initScrapProductInfo(Stockdrug stockDrug){
         ...
         * @return
         */
        public ScrapInfoDialog createTwoButtonDialog() {
            showTwoButton();
        ...
        private void create() {
            ...
            if(stockDrug!=null){
                //库存id
                ((TextView)layout.findViewById(R.id.tv_scrap_dialog_stock_name)).setText(stockDrug.getName()+"");
                //序列号
                ((TextView)layout.findViewById(R.id.tv_scrap_dialog_stock_serialNo)).setText(stockDrug.getStockNo());
                //资产状态
                ((TextView)layout.findViewById(R.id.tv_scrap_dialog_stock_max)).setText(stockDrug.getMaxNum()+"");
                //资产名称
                ((TextView)layout.findViewById(R.id.tv_scrap_dialog_stock_opuser)).setText(stockDrug.getOpUser());
